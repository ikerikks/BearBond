...
const PostDetailsPage = () => {
  ...
  const { data: post, isPending, isError } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await axios.get(`/api/posts/status/${posterId}`);
        const { data } = res;
  ...
  return (
    <>
      ...
      {isError && (
        <div className="flex justify-center">No post found</div>
      )}
      {post && (
        <div className="pb-[20%]">
          <div className="flex gap-4 items-center px-3 pt-3 pb-4 bg-white/70 sticky top-0 backdrop-blur-lg z-10">
            <Link className="md:hover:bg-red-200 p-2 rounded-full" onClick={() => { navigate(-1) }}>
              <FaArrowLeft className="size-5" />
            </Link>
            <p className="font-bold text-xl text-neutral">Post</p>
          </div>
          {/* post */}
          <div
          ...
          >
              ...
              {post.user._id === authUser._id && (
                <button
                  className={`flex justify-center items-center text-xl text-neutral size-8 rounded-full 
                        ${!deletingPost && 'md:hover:bg-red-200'}`}
                  onClick={() => {
                    deletePost();
                    navigate(-1);
                  }}
                >
                  {deletingPost
                    ...
                    : <PiTrashBold className="text-primary text-md" />
                  }
                </button>
              )}
            </div>
            ...
            <p className="text-gray-700 text-sm font-semibold opacity-75 underline-offset-8 px-3">
          ...
          <div className="flex flex-col">
            {post.myComments && post.myComments.map((comment) => (
              <div key={comment._id} className="flex flex-col gap-2 px-3 py-4 items-start border-b">
                ...
                {!(comment.user._id === authUser._id) &&
                  (<button
                    className="flex items-center gap-1 text-primary text-sm font-bold cursor-pointer rounded-full md:hover:bg-red-200"
                    onClick={handleChat}
                  >
                    Chat
                    <PiChatCenteredDotsBold className="text-lg" />
                  </button>)
                }
